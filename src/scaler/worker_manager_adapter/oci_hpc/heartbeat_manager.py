"""
OCI Container Instance Heartbeat Manager.

Sends periodic heartbeats to the scheduler with worker status.

Analogous to AWSBatchHeartbeatManager but for OCI Container Instance adapter.
"""
...
class OCIContainerInstanceHeartbeatManager(Looper, HeartbeatManager):
    """
    Heartbeat manager for OCI Container Instance worker adapter.

    Periodically reports worker status (resource usage, task queue depth,
    and latency) to the Scaler scheduler.
    """
    ...
    def __init__(
        ...
        self._task_manager: Any = None  # OCIHPCTaskManager
    ...
    def register(
        ...
        worker_task_manager: Any,  # OCIHPCTaskManager
    ...
    async def routine(self) -> None:
        ...
        # Agent resource (cpu percentage, rss memory)
        agent_resource = Resource.new_msg(cpu=agent_cpu, rss=agent_rss)

        # Processor status for the adapter (simulated as a single remote processor).
        # pid=0 since OCI Container Instances run remotely; initialized=True means ready.
